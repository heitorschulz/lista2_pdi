...
def histograma(imagem,nome):
    ...
    #plt.show()
    plt.savefig('output/teste_2_hist_'+nome+'.png', dpi=100)
...
def main():
    ...
    R_array=np.clip(image[:,:,0],20,80)
    G_array=np.clip(image[:,:,1],20,80)
    B_array=np.clip(image[:,:,2],20,80)
    ...
    img.save('output/teste_2_equalizacao_histograma.jpg')
